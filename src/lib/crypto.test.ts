...
describe('Crypto Service', () => {
  ...
  describe('generateUserSecret', () => {
    it('should generate a deterministic secret for same inputs', () => {
      const secret1 = generateUserSecret(testUserId, testSessionId, testEncryptionSecret);
      const secret2 = generateUserSecret(testUserId, testSessionId, testEncryptionSecret);

      expect(secret1).toBe(secret2);
    });

    it('should generate different secrets for different users', () => {
      const secret1 = generateUserSecret('user-1', testSessionId, testEncryptionSecret);
      const secret2 = generateUserSecret('user-2', testSessionId, testEncryptionSecret);

      expect(secret1).not.toBe(secret2);
    });

    it('should generate different secrets for different sessions', () => {
      const secret1 = generateUserSecret(testUserId, 'session-1', testEncryptionSecret);
      const secret2 = generateUserSecret(testUserId, 'session-2', testEncryptionSecret);

      expect(secret1).not.toBe(secret2);
    });

    it('should generate different secrets for different app secrets', () => {
      const secret1 = generateUserSecret(testUserId, testSessionId, 'app-secret-1');
      const secret2 = generateUserSecret(testUserId, testSessionId, 'app-secret-2');
  ...
  describe('Integration tests', () => {
    it('should encrypt and decrypt with generated user secret', () => {
      const userSecret = generateUserSecret(testUserId, testSessionId, testEncryptionSecret);
    ...
    it('should not decrypt with different user session', () => {
      const userSecret1 = generateUserSecret('user-1', 'session-1', testEncryptionSecret);
      const userSecret2 = generateUserSecret('user-2', 'session-2', testEncryptionSecret);
