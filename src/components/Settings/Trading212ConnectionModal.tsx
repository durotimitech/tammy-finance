    ...
    try {
      ...
      if (!testResponse.ok) {
        if (testResponse.status === 401) {
          throw new Error(
            "Invalid API key. Please check your Trading 212 API key and try again.",
          );
        } else {
      ...
      if (checkResponse.ok) {
        ...
        credentialExists =
          existingCreds.credentials?.some(
            (cred: { name: string }) => cred.name === "trading212",
          ) || false;
      }
          ...
          {!isSuccess ? (
            ...
            >
              ...
              <p className="text-sm text-gray-500">
                Your Trading 212 account has been connected.
              </p>
            </motion.div>
          )}
        </motion.div>
