...
export default function NetWorthChart({ refreshKey }: NetWorthChartProps) {
  ...
  const { data: historyData, isLoading } = useQuery({
    queryKey: [...queryKeys.history, timeRange, startDate.toISOString().split('T')[0], refreshKey],
    queryFn: () =>
  ...
  const getTrendIcon = () => {
    ...
    if (trend.trend === 'up') {
      return <TrendingUp className="w-5 h-5" style={{ color: 'var(--green)' }} />;
    } else if (trend.trend === 'down') {
      return <TrendingDown className="w-5 h-5" style={{ color: 'var(--red)' }} />;
    }
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: '#76c893' }} />
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: '#0096c7' }} />
            ...
            {(['30d', '90d', '1y', 'all'] as const).map((range) => (
              <motion.button
                ...
                className={`px-2 sm:px-3 py-1 text-xs sm:text-sm rounded-md transition-colors whitespace-nowrap ${
                  timeRange === range ? 'text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                }`}
                style={timeRange === range ? { backgroundColor: 'var(--secondary)' } : {}}
              >
          ...
          <div className="mt-4 grid grid-cols-3 gap-4 pt-4 border-t border-gray-200">
            ...
            <div>
              ...
              <p className="text-lg font-semibold" style={{ color: 'var(--red)' }}>
                {formatCurrency(data[data.length - 1]?.liabilities || 0, currency)}
              </p>
            </div>
          </div>
        )}
