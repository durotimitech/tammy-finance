  ...
  try {
    ...
    const history: BudgetHistorySummary[] = await Promise.all(
      (budgetMonths || []).map(async (month) => {
        ...
        if (expensesError) {
          console.error(
            `Error fetching expenses for ${month.year}-${month.month}:`,
            expensesError,
          );
        }
        ...
        const expensesWithGoals =
          expenses?.map((expense: SupabaseExpenseResponse) => {
            ...
            return {
              ...
              goal_name:
                (goalData?.category_name
                  ? String(goalData.category_name)
        ...
        return {
          ...
          net_savings:
            Number(month.total_income) - Number(month.total_expenses),
