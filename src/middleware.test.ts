...
describe('Middleware', () => {
  ...
  it('should call updateSession with the request', async () => {
    const mockRequest = new NextRequest(new URL('http://localhost:3000/dashboard'));
  ...
  it('should pass through the response from updateSession', async () => {
    const mockRequest = new NextRequest(new URL('http://localhost:3000/api/assets'));
    const redirectResponse = NextResponse.redirect(new URL('http://localhost:3000/auth/login'));
...
describe('updateSession', () => {
  ...
  const { updateSession: actualUpdateSession } = require('./lib/supabase/middleware');
  ...
  it('should allow access to protected routes when user is authenticated', async () => {
    ...
    const request = new NextRequest(new URL('http://localhost:3000/dashboard/assets'));
    const response = await actualUpdateSession(request);
